        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('admin.layanan.update', $service) }}">
                        @csrf
                        @method('PATCH')
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="requirements" name="requirements" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" rows="5" required>{{ old('requirements', $service->requirements) }}</textarea>
                            <x-input-error :messages="$errors->get('requirements')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="flow" :value="__('Alur Proses (Pisahkan dengan baris baru)')" />
                            <textarea id="flow" name="flow" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" rows="5" required>{{ old('flow', $service->flow) }}</textarea>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                             <div>
                                ...
                                <x-text-input id="duration" class="block mt-1 w-full" type="text" name="duration" :value="old('duration', $service->duration)" />
                                <x-input-error :messages="$errors->get('duration')" class="mt-2" />
                            </div>
                             <div>
                                <x-input-label for="cost" :value="__('Biaya')" />
                                <x-text-input id="cost" class="block mt-1 w-full" type="text" name="cost" :value="old('cost', $service->cost)" />
